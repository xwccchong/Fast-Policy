...
class Karras_Scheduler():
    ...
    def heun_solver(self, model, samples, t, next_t, clamp = False):
        ...
        denoisedy, encoder_feature1,energy = self.calc_out(model, y, t, clamp = clamp)
        dy = (y - denoisedy) / append_dims(t, dims)


        y_next = samples + step * dy 
        denoisedy_next, encoder_feature2,energy_next = self.calc_out(model, y_next, next_t, clamp = clamp)
        ...
        return y_next,encoder_feature1,encoder_feature2,energy,energy_next
