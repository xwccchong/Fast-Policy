...
# You should always set training.inference_mode=false while training.

class EDMWorkspace(BaseWorkspace):
    include_keys = ['global_step', 'epoch']

    def __init__(self, cfg: OmegaConf, output_dir=None):
        cfg.policy.inference_mode = cfg.training.inference_mode # false
    ...
    def run(self): 
        ...
        # device transfer 
        """
        There is a small bug here, and multi-card parallelism is not implemented. 
        At the same time, the key value is modified, but I wrote a function to modify it later.
        """
        self.model = nn.DataParallel(self.model, device_ids=[0]) 
        self.ema_model = nn.DataParallel(self.ema_model, device_ids=[0]) 
        ...
        timesteps = torch.arange(0, self.model.module.noise_scheduler.bins, device=device)
        b, next_b = self.model.module.noise_scheduler.timesteps_to_times(timesteps[:-1]), self.model.module.noise_scheduler.timesteps_to_times(timesteps[1:])
                ...
                if not cfg.training.inference_mode: 
                    ...
                    if (self.epoch % cfg.training.sample_every) == 0: 
                        with torch.no_grad():
                            ...
                            print("==================evaling training batch==================")
                            start_time = time.time()
                            # result = policy.predict_action(obs_dict)
                            result = policy.faster_edm_predict_action(obs_dict)
                            t = time.time() - start_time
