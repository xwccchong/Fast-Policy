    ...
    def calc_out(self, model, trajectory: torch.Tensor, times: torch.Tensor, clamp=False): 
        if isinstance (times, list): 
            ...
            if self.scaling == "boundary":
                c_skip, c_out, c_in = [append_dims(c, trajectory.ndim) for c in self.get_scalings_for_boundary_condition(key_time)]
            elif self.scaling == "no_boundary":
                c_skip, c_out, c_in = [append_dims(c, trajectory.ndim) for c in self.get_scalings(key_time)]
            else:
                raise ValueError(f"Unknown scaling {self.scaling}")
            
            rescaled_times = []
            for t in times:       
                rescaled_times.append(1000 * 0.25 * torch.log(t + 1e-44)) # *1000 to make it more stable 
            
            model_output = model(trajectory * c_in, rescaled_times)
            trajectory1 = copy.deepcopy(trajectory)
            processed_batches = []
            for _ in range(len(times)):
                copy_trajectory = trajectory1.repeat(1,1,1)
                processed_batches.append(copy_trajectory)
            trajectory_cat = torch.cat(processed_batches, dim=0) 
            out = model_output * c_out + trajectory_cat * c_skip
