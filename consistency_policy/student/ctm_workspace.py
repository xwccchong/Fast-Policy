...
class CTMWorkspace(BaseWorkspace):
    ...
    def __init__(self, cfg: OmegaConf, output_dir=None):
        ...
        cfg.training.resume_path = "/data/wts/consistency_policy_output/ctm/can/checkpoints/epoch=0450-test_mean_score=0.980.ckpt"
        ...
        with JsonLogger(log_path) as json_logger:
            for local_epoch_idx in range(cfg.training.num_epochs):
                ...
                if not cfg.training.inference_mode:
                    ...
                    steps_per_epoch = len(train_dataloader) 
                    stepping_batches = math.ceil(steps_per_epoch / cfg.training.gradient_accumulate_every) * self.p_epochs 
                ...
                if (self.epoch % cfg.training.rollout_every) == 0 and cfg.training.online_rollouts: 
                    policy.chaining_steps = cfg.training.val_chaining_steps # 1
                ...
                if (self.epoch % cfg.training.checkpoint_every) == 0: 
                    # checkpointing
                    if cfg.checkpoint.save_last_ckpt: # true
                        self.save_checkpoint()
                    if cfg.checkpoint.save_last_snapshot: # false
                        self.save_snapshot()
